 ...
 * @LastEditTime: 2026-09-02 17:52:52
 * @License: GPL 3.0
 */
#include <cstdint>
...
bool TDisplayP4Device::ReadBatteryManagementStatus(
  ...
  *status = BatteryManagementStatus();
  ...
  status->charge_percent = charge_percent == UINT16_MAX ? 0 : charge_percent;
  ...
  status->pack_temperature_c = driver_.chip().bq27220->GetTemperatureCelsius();
  status->chip_temperature_c =
      driver_.chip().bq27220->GetChipTemperatureCelsius();

  const bool fully_charged = status_read && battery_status.flag.full_charged;
  const bool idle_or_charging = status_read && !battery_status.flag.discharging;
  ...
  status->full_discharged = status_read && battery_status.flag.full_discharged;
  return true;
}
...
bool TDisplayP4Device::SetBatteryCapacityMah(int capacity_mah) {
  ...
  if (driver_.IsBq27220Ready() && !driver_.chip().bq27220->SetBatteryCapacity(
