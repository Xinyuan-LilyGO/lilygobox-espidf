...
void TDisplayP4AirDevice::RequestCameraPreviewStop() {
  camera_preview_.stop_requested.store(true);
}
...
bool TDisplayP4AirDevice::InitializeCameraPreview() {
  ...
  while (!camera_preview_.stop_requested.load() &&
    ...
    const uint32_t delay_ms = std::min(
        kCameraPowerCycleOffDelayMs, CameraStartupRemainingMs(startup_tick));
    if (delay_ms > 0) {
      vTaskDelay(pdMS_TO_TICKS(delay_ms));
    }
  }
...
TDisplayP4AirDevice::CameraStartupAttemptResult
TDisplayP4AirDevice::InitializeCameraPreviewAttempt(TickType_t startup_tick) {
